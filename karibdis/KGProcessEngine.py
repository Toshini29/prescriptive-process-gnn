...
from pyshacl.consts import SH_ValidationResult, SH_resultSeverity, SH_value, SH_resultMessage, SH_Info
import datetime
...
class KGProcessEngine:
    ...
    def extract_karibdis_scores(self):
        """
        Extract Karibdis scores for all open decisions in the current process knowledge graph.
        Returns a dict: {decision_subject: [(score, option, verdict), ...]}
        """
        scores = {}
        for decision in self.open_decisions():
            top_options = decision.get_top_k_results(k=5)
            scores[str(decision.subject)] = top_options
        return scores
